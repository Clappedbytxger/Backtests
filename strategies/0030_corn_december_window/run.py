"""Strategy 0030 — Corn December window 1 Dec–31 Dec (Seasonax lead).

Full honesty harness (as 0028) PLUS the now-mandatory roll-day check (from 0029),
on the CBOT corn front-month future `ZC=F`. User-supplied Seasonax window: **1 Dec
to 31 Dec** (single calendar year, no wrap; ~21 trading days). IS/OOS split at 1 Jan
(never bisects a December window).

Why corn passes the first smell test that NG failed: the December strength is a
*broadly distributed* daily drift (~+0.26%/day, 56% of days up across the month),
not a single roll-day spike — the late-November Dec->Mar roll zone is actually flat.
Corn delivery months are Mar/May/Jul/Sep/Dec; the continuous series rolls off the
December contract around late Nov / mid Dec, so a roll/expiry zone *can* fall inside
the window — hence the roll-exclusion test is run regardless (lesson 0029).

Macro rationale: the US harvest low (Sep/Oct supply flood) is past by December;
prices firm into year-end on demand, export pace and the South-American weather
premium (Brazil/Argentina summer crop). Caveat: corn is *the* most-researched ag
seasonal — a real edge often lives only in the calendar spread, not flat-price
front-month — so the permutation test and roll check carry extra weight.

Guards: non-positive close (0005) + frozen-feed check (<50 distinct closes/yr, 0025).

Run:
    .venv/Scripts/python.exe strategies/0030_corn_december_window/run.py
"""
...
TICKER = "ZC=F"
NAME = "Mais (Corn-Future)"
START_MD = (12, 1)          # 1 December — pre-committed from Seasonax
END_MD = (12, 31)           # 31 December (same year)
COST_MODEL = IBKR_FUTURES

# Suspected roll/expiry zone inside the window: the Dec contract's last trading day
# is ~the business day before the 15th -> mid December. Tight zone around it.
ROLL_ZONES = [((12, 11), (12, 16))]
...
def date_window_signal(index, start_md=START_MD, end_md=END_MD,
                       ...
                       name="corn_dec") -> pd.Series:
    """Long (1.0) over 1 Dec -> 31 Dec each year, else flat (wrap-aware).

    Decision-time signal; the engine applies the T+1 shift, so no look-ahead.
    ``start_shift``/``end_shift`` probe robustness; ``exclude_roll`` forces flat on
    the suspected roll-zone days to isolate the non-roll contribution.
    """
...
def roll_check(prices: pd.DataFrame, asset_ret: pd.Series, split_date: str) -> dict:
    ...
    # Decompose mean trade PnL into roll vs non-roll legs.
...
def main() -> None:
    print(f"Strategy 0030 — {NAME} December window (1.12..31.12, {TICKER})")
    ...
    plotting.savefig(
        plotting.plot_equity(
            ...
            title=f"0030 {NAME} — Dezemberfenster 1.–31.12. vs. Buy & Hold (gesamt)",
            strategy_label="Fenster (long 1.–31.12., sonst flat)",
            benchmark_label=f"{NAME} Buy & Hold",
            caption=("Nur ~21 Handelstage/Jahr long im CBOT-Mais-Future, sonst flat, netto nach "
                     "IBKR-Futures-Kosten. Fenster von Seasonax auf dieser Historie gewählt → in-sample "
                     "geschönt. Aussagekräftig: Permutation, OOS-Hälfte, Robustheit, Roll-Tag-Check. "
                     "Vorbehalt: Mais ist der überforschte Ag-Seasonal — Edge evtl. nur im Kalenderspread.")),
    ...
    ax.set_ylabel("Netto-Rendite pro Dez-Trade (%)")
    ax.set_title(f"0030 {NAME} — Rendite je Dezember-Trade (netto)", fontsize=12, fontweight="bold")
    plotting._add_caption(fig, f"Jeder Balken = ein Dezember-Trade (~21 Tage). Trefferquote "
    ...
    ax.set_xlabel("Verschiebung Start (Tage, 0 = 1. Dez.)")
    ax.set_ylabel("Verschiebung Ende (Tage, 0 = 31. Dez.)")
    ...
    ax.set_title(f"0030 {NAME} — Robustheit je Fenster-Verschiebung", fontsize=12, fontweight="bold")
    plotting._add_caption(fig, "Netto-Expectancy/Trade (%) bei Verschiebung von Start (Spalte)/Ende "
                               "(Zeile). Schwarz = Seasonax-Fenster. Permutation + Roll-Check entscheiden.")
    ...
    with open(RESULTS / "card.json", "w") as fh:
        json.dump({"id": "0030", "label": f"{NAME} Dezemberfenster 1.-31.12.",
    ...
    # --- Console ---
    print(f"\n  Rule: long 1.12..31.12 each year, {TICKER}\n")
    ...
    print("\n  ROLL-DAY CHECK (mandatory, lesson 0029) — roll zone 11.-16.12.:")
