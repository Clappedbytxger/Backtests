"""Strategy 0016 — Charter Communications summer window (Seasonax lead).

The user found this in the Seasonax trial: Charter Communications (CHTR, ISIN
US16119P1084) appears strong from **4 June to 22 July** each year. Seasonax picks
that window by scanning the *whole* price history for the best start/end pair, so
the headline stat is in-sample by construction — exactly the over-fit trap that
sank 0005/0008/0011. This script tests the lead honestly:

  * Full sample = the optimistic "Seasonax view" (window chosen on this data).
  * In-sample / out-of-sample split (median year) to check the window is not just
    a first-half artifact.
  * Per-year breakdown — how many years actually carry the effect.
  * Robustness grid — shift the window's start and end by +/- days. A real
    seasonal effect is a contiguous green plateau; a knife-edge is noise.
  * Costs (IBKR US stock), permutation test, bootstrap Sharpe CI, t-test and a
    Deflated Sharpe charged the search width (Seasonax scanned many windows).

Macro caveat (hard rule): a broadband/cable operator has no obvious supply/demand
season in June-July. The only half-plausible cause is a pre-Q2-earnings drift
(Charter reports late July, just after the window) — weak. Treat as suspect /
data-mined until robustness + OOS argue otherwise.

Run:
    .venv/Scripts/python.exe strategies/0016_charter_summer_window/run.py
"""
...
START_MD = (6, 4)          # 4 June  — pre-committed from the Seasonax trial
END_MD = (7, 22)           # 22 July
COST_MODEL = IBKR_DEFAULT  # US single stock


def date_window_signal(index, start_md=START_MD, end_md=END_MD,
                       start_shift=0, end_shift=0, name="chtr_summer") -> pd.Series:
...
def main() -> None:
    print(f"Strategy 0016 — {NAME} summer window "
    ...
    plotting.savefig(
        plotting.plot_equity(
            ...
            title=f"0016 {NAME} — Sommerfenster 4.6.–22.7. vs. Buy & Hold (gesamt)",
            strategy_label="Sommerfenster (long 4.6.–22.7., sonst flat)",
            benchmark_label=f"{NAME} Buy & Hold",
            caption=("Nur ~33 Handelstage/Jahr long in der Aktie, sonst flat, netto nach "
    ...
    ax.set_title(f"0016 {NAME} — Rendite je Jahres-Trade (4.6.–22.7., netto)",
                 fontsize=12, fontweight="bold")
    plotting._add_caption(fig, (
        f"Jeder Balken = ein Jahres-Trade (~33 Tage long). Grün = Gewinn. Trefferquote "
        f"{win:.0%} über {len(trades)} Trades. Wenige Trades (1/Jahr) bleiben die "
        f"strukturelle Hauptschwäche eines Einzelaktien-Saisonfensters."))
    ...
    ax.set_xlabel("Verschiebung Startdatum (Kalendertage, 0 = 4. Juni)")
    ax.set_ylabel("Verschiebung Enddatum (Kalendertage, 0 = 22. Juli)")
    ...
    ax.set_title(f"0016 {NAME} — Robustheit: Expectancy je Fenster-Verschiebung",
                 fontsize=12, fontweight="bold")
    plotting._add_caption(fig, (
    ...
    summary = {
        "ticker": TICKER, "name": NAME, "isin": "US16119P1084",
    ...
    card = {
        "id": "0016", "label": f"{NAME} Sommerfenster 4.6.-22.7.",
