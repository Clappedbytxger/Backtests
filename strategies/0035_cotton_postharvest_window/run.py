"""Strategy 0035 — Cotton post-harvest window 21 Nov–29 Dec (Seasonax lead).

Full honesty harness (0028) + mandatory roll-day check (0029), on ICE cotton No. 2
front-month future `CT=F`. User-supplied Seasonax window: **21 Nov to 29 Dec** (single
calendar year, ~26 trading days). IS/OOS split at 1 Jan.

Macro rationale: the Northern-Hemisphere cotton harvest runs Sep-Nov; by late Nov the
harvest-supply pressure is largely past and prices can firm into year-end on demand and
next-season acreage uncertainty. Caveat (lesson 0017): cotton carries moderate drift, so
a long multi-week window risks the drift trap — the permutation test must clear it.

ROLL RISK (lesson 0029): cotton contracts are Mar/May/Jul/Oct/Dec; in November the front
is the Dec contract, whose first-notice/roll to March falls ~late November — *inside* the
window's early part. The roll-day exclusion test (23-30 Nov) is therefore run regardless.

Guards: non-positive close (0005) + frozen-feed (<50 distinct/yr, 0025).

Run:
    .venv/Scripts/python.exe strategies/0035_cotton_postharvest_window/run.py
"""
...
TICKER = "CT=F"
NAME = "Baumwolle (Cotton-Future)"
START_MD = (11, 21)         # 21 November — pre-committed from Seasonax
END_MD = (12, 29)           # 29 December (same year)
COST_MODEL = IBKR_FUTURES
ROLL_ZONES = [((11, 23), (11, 30))]    # Dec contract -> Mar roll ~late November
...
def date_window_signal(index, start_shift=0, end_shift=0, exclude_roll=False, name="cotton") -> pd.Series:
...
def main():
    print(f"Strategy 0035 — {NAME} window ({START_MD[1]}.{START_MD[0]}..{END_MD[1]}.{END_MD[0]}, {TICKER})")
    ...
    plotting.savefig(plotting.plot_equity(
        ...
        title=f"0035 {NAME} — Fenster 21.11.–29.12. vs. Buy & Hold (gesamt)",
        strategy_label="Fenster (long 21.11.–29.12., sonst flat)", benchmark_label=f"{NAME} Buy & Hold",
        caption=("~26 Handelstage/Jahr long im ICE-Cotton-Future, sonst flat, netto. Fenster Seasonax-"
                 "gemined → in-sample geschönt. Aussagekräftig: Permutation, OOS, Robustheit, Roll-Tag-Check "
                 "(Dez→März-Roll Ende Nov). Treiber-These: Erntedruck (Sep–Nov) durch, Festigung ins Jahresende. "
                 "Vorsicht Drift-Falle (0017): Baumwolle driftet moderat.")),
    ...
    ax.set_title(f"0035 {NAME} — Rendite je Jahresend-Trade (netto)", fontsize=12, fontweight="bold")
    plotting._add_caption(fig, f"Jeder Balken = ein Trade (~26 Tage). Trefferquote {(trades['pnl']>0).mean():.0%} über {len(trades)} Trades.")
    ...
    ax.set_xlabel("Verschiebung Start (Tage, 0 = 21.11.)"); ax.set_ylabel("Verschiebung Ende (Tage, 0 = 29.12.)")
    ...
    ax.set_title(f"0035 {NAME} — Robustheit je Fenster-Verschiebung", fontsize=12, fontweight="bold")
    plotting._add_caption(fig, "Netto-Expectancy/Trade (%) bei Start-(Spalte)/Ende-(Zeile)-Verschiebung. Schwarz = Seasonax-Fenster. Bei driftendem Asset Plateau mit Vorsicht; Permutation + Roll-Check entscheiden.")
    ...
    print(f"\n  ROLL-DAY CHECK (mandatory 0029) — roll zone 23.-30.11.:")
