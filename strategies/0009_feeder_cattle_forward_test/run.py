"""Strategy 0009 — Feeder cattle spring window: a FORWARD TEST of the 0008 lead.

0008 scanned 156 short windows per commodity in-sample (2000-2015) across 20 fresh
futures and found ONE that also held out-of-sample: feeder cattle (GF=F) long in
ISO week 21 (~late May). Because it was the best of many windows over 20 assets, it
could be the lucky 1-of-20 false positive. The honest next step is identical to the
gasoline escalation in 0006: a **forward test** of the rule fixed on old data, run
on data that played NO role in choosing it.

Splits used here:
  * **Discovery** 2000-2015 — the only data that picked "week 21". (Context.)
  * **Forward**   2016-2026 — unseen by the window choice; the forward test.
  * **Recent holdout** last 5 years — the purest forward slice.

Macro rationale for the window: feeder cattle (young cattle placed on pasture to
graze before feedlot finishing) see demand pressure in **late spring** — start of
the grazing season plus grilling/beef-demand seasonality into Memorial Day (late
May ~ ISO week 21).

Because a forward test evaluates ONE pre-committed rule, there is no search burden:
the Deflated Sharpe uses n_trials=1 (≈ probabilistic Sharpe).
...
Run:
    .venv/Scripts/python.exe strategies/0009_feeder_cattle_forward_test/run.py
"""
...
TICKER = "GF=F"
NAME = "Mastrind (Feeder-Cattle-Futures)"
ISO_WEEK = 21            # pre-committed from 0008 in-sample scan
BASE_OFFSET = 0          # entry on the first trading day of week 21
...
def event_signal(index, iso_week=ISO_WEEK, entry_offset=0, hold_days=BASE_HOLD,
    ...
    look-ahead. ``entry_offset`` < 0 enters earlier, > 0 later.
    """
...
def main() -> None:
    print(f"Strategy 0009 — Feeder cattle week-{ISO_WEEK} forward test ({TICKER})")
    ...
    plotting.savefig(
        plotting.plot_equity(
            (1 + fwd_rets).cumprod(),
            title=f"0009 Mastrind Saison-Fenster KW{ISO_WEEK} — Forward-Test Kapitalkurve (ab 2016)",
            strategy_label=f"Mastrind KW{ISO_WEEK} (long, ~1 Woche/Jahr)",
            caption=("Nur diese Strategie: jedes Jahr ~5 Handelstage ab Anfang ISO-Woche 21 "
                     "long im Feeder-Cattle-Future, sonst flat, netto nach Kosten. Die Regel "
                     "wurde 2000–2015 fixiert; gezeigt ist die Auszahlung auf den danach "
                     "folgenden, bei der Fenster-Wahl ungesehenen Jahren (echter Forward-Test).")),
        PLOTS / "forward_equity.png")

    # Plot 2: per-year trade returns (forward).
    ...
    ax.set_title(f"0009 Mastrind KW{ISO_WEEK} — Rendite je Jahres-Trade (Forward, netto)",
                 fontsize=12, fontweight="bold")
    win = (trades["pnl"] > 0).mean()
    plotting._add_caption(fig, (
        f"Jeder Balken = ein Jahres-Trade (~5 Tage long Ende Mai). Grün = Gewinn. "
    ...
    ax.set_xlabel("Einstiegs-Offset (Handelstage relativ zum Start von KW21)")
    ...
    ax.set_title(f"0009 Mastrind KW{ISO_WEEK} — Robustheit: Expectancy je Einstieg/Haltedauer (Forward)",
                 fontsize=12, fontweight="bold")
    plotting._add_caption(fig, (
    ...
    fm = base_fwd["metrics"]
    card = {
        "id": "0009", "label": f"Mastrind Saison KW{ISO_WEEK} (Forward-Test)",
